    ...
    export let  makePolygon = (env:Environment.Env, args:any[]) => {
        switch(args.length){
            ...
            case 1:
                ...
                IO.pushSVG(`<polygon points="${Polygon.polygonString(arg.arrayValue())}" stroke="black" stroke-width="1"/>`);
                break;
            ...
            case 2:
                ...
                if(typeof arg !== 'object') throw new Error(`TypeError: Expected list argument, got ${typeof arg}`);
                IO.pushSVG(`<polyline points="${Polygon.polygonString(arg.arrayValue())}" stroke="rgb(${color.getItem('r')},${color.getItem('g')},${color.getItem('b')})" stroke-width="1"/>`);
                break;
            default:
